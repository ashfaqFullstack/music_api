...
const uploadMusic = catchAsync(async (req, res) => {
  ...
  const payload = {
    ...
    musicImage: req.files['musicImage'] ? req.files['musicImage'][0].path.replace('/public', '') : null,
    musicAudio: req.files['musicAudio'] ? req.files['musicAudio'][0].path.replace('/public', '') : null,
